package nc.impl.wsh.scll.scllmaster;
...
import nc.vo.scll.scll.ScllSlave0VO;
...
public class  ScllMasterVOServiceImpl extends ServiceSupport implements IScllMasterVOService {
	...
	public String[] listScllMasterVOPkByCond(String condition,String[] orderPath) throws BusinessException{
		...
		sql.append(" select pk_scllmaster from ").append(ScllMasterVO.getDefaultTableName());
	...
	public AggScllMasterVO preAddAggScllMasterVO(AggScllMasterVO vo,Map<String,Object> userJson) throws BusinessException{
		...
		initDefaultData((ScllMasterVO)getMainVO(vo));
	...
	public AggScllMasterVO preAddAggScllMasterVO(Map<String,Object> userJson) throws BusinessException{
		AggScllMasterVO result = null;

		ScllMasterVO mainvo = new ScllMasterVO();
		//设置默认值
		initDefaultData(mainvo);
		AggScllMasterVO aggvo = new AggScllMasterVO();
		aggvo.setParent(mainvo);
		result = aggvo;
		return preAddAggScllMasterVO(result,userJson);
	}
	...
	public AggScllMasterVO copyAggScllMasterVO(String pk) throws BusinessException{
		...
		AggScllMasterVO vo = dao.findByPk(AggScllMasterVO.class, pk, false);
		...
		ScllSlave0VO[] scllSlave0VOs = (ScllSlave0VO[])vo.getChildren(ScllSlave0VO.class);
		if(scllSlave0VOs!=null && scllSlave0VOs.length>0){
			Arrays.stream(scllSlave0VOs).forEach(subvo->{
	 ...
	 * @param vos
	 */
	private void setAuditInfo(AggScllMasterVO... vos) throws BusinessException {
		if(ArrayUtils.isNotEmpty(vos)) {
			UFDateTime now = new UFDateTime();
			for(AggScllMasterVO vo : vos) {
				String pk = getVOPrimaryKey(vo);
				if(StringUtils.isEmpty(pk)){
					//设置创建人创建时间
	 ...
	 * @param vos
	 */
	private void setDefaultVal(AggScllMasterVO... vos) throws BusinessException {
		setAuditInfo(vos);
		//其他默认值处理
	}
	...
	public AggScllMasterVO[] deleteAggScllMasterVOs(Map<String,String> tsMap) throws BusinessException{
		AggScllMasterVO[] vos = dao.listByPk(AggScllMasterVO.class,tsMap.keySet().toArray(new String[0]));
		validate(vos,tsMap);
		dao.delete(vos,true);
		return vos;
	}
	...
	//校验  包括ts校验  已提交校验
	private void validate(AggScllMasterVO[] vos,Map<String,String> tsMap) throws BusinessException{
	...
	public String[] queryChildPksByParentId(Class childClazz, String parentId) throws BusinessException{
		String cond = " pk_scllmaster = '" + parentId + "' ";
